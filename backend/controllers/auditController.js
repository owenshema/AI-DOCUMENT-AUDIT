...
const exportAuditReport = async (req, res) => {
  try {
    ...
    // ── Branded header (shared DocAudit AI layout) ──
    let y = layout.drawHeader(doc);

    const periodLabel = `${report.periodStart ? layout.fmtDate(report.periodStart) : '—'} - ${report.periodEnd ? layout.fmtDate(report.periodEnd) : '—'}`;
    const preparedBy = report.User ? `${report.User.fullName} (${report.User.role})` : 'System';

    // ── Report details block ──
    y = layout.drawReportDetails(doc, y, [
      ['Report Type',   (report.reportType || '').replace(/_/g, ' ')],
      ['Status',        report.status],
      ['Report Period', periodLabel],
      ['Prepared By',   preparedBy],
      ['Date Generated', layout.fmtDate(report.createdAt)],
    ], report.title);

    // ── Centered title ──
    y = layout.drawCenteredTitle(doc, y, `${report.title || 'Audit Report'} – ${layout.monthYear(report.periodEnd || report.createdAt)}`);
    ...
    const metricsLine = `Documents: ${report.metrics?.totalDocuments ?? 0}     ·     AI Analyses: ${report.metrics?.totalAnalyses ?? 0}     ·     Checks: ${report.metrics?.totalChecks ?? 0} (${report.metrics?.passRate ?? 0}% pass rate)`;
    doc.fontSize(9).font('Helvetica').fillColor('#6b7280').text(metricsLine, 50, y, { width: 495 });
    y += 22;
    ...
    // ── Signature block + branded "Generated By" footer ──
    layout.drawSignatureBlock(doc, y, preparedBy);
