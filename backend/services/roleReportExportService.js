...
function writePdf(report, res) {
  ...
  const isClientReport = String(report.exportedByRole || '').toLowerCase() === 'client';
  if (!isClientReport && report.preparedBy) {
    detailPairs.push(['Generated by', report.preparedBy]);
  }
  ...
  // 5. Signature block (staff only) + 6. Footer
  if (!isClientReport) {
